...
This repository showcases a professional data engineering and business intelligence pipeline. It automates the generation of complex financial datasets, manages data persistence through a relational **SQL Server** backend, and delivers strategic insights via a high-performance **Power BI** dashboard.

By moving beyond static files, this architecture simulates a real-world enterprise environment where data flows from a scripted synthetic source into a live data warehouse for robust reporting and analytics.

---

## 🏗️ Architecture & Data Flow

1. **Extraction & Generation (Python)**  
   A custom Python ETL script generates ~2,500 rows of transactional data with realistic geographic distribution (Windsor, Toronto, Detroit, Kitchener, Ottawa).

2. **Storage & Persistence (SQL Server)**  
   Using `SQLAlchemy` and `pyodbc`, the script performs high-speed batch inserts into a local **SQL Server (SSMS)** instance.

3. **Modeling (Star Schema)**  
   Data is structured into a professional Star Schema within Power BI, utilizing a dedicated `Dim_Date` table for optimized performance.

4. **Analytics (DAX)**  
   Advanced DAX measures for Budget Variance, Month-over-Month (MoM) growth, and Cumulative "Burn Rate" analysis.
...
- **Automation**: The Python script handles the creation of `Fact_Transactions_Messy` and `Fact_Budget_Messy` tables.
...
3. **Run ETL**: Execute `script/datagenerator_sql.py` to populate the SQL tables with generated messy data.
...
- `/script/datagenerator_sql.py`: ETL script for data generation and SQL injection.
- `/images/`: High-resolution images of the dashboard.
- `Financial_Intelligence_V4.pbix`: The master Power BI dashboard file.
